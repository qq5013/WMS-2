...
namespace Business.Common.Toolkit
{
    public class RadixConvertHelper
    {
        ...
        private static long x2h(string value, int fromBase)
        {
            ...
            var sDigits = new string(rDigits, 0, fromBase);
            long result = 0;
            ...
            for (int i = 0; i < value.Length; i++)
            {
                ...
                else
                {
                    try
                    {
                        ...
                        result += (long) Math.Pow(fromBase, i)*getcharindex(rDigits, value[value.Length - i - 1]);
                            //   2
                    }
                    catch
                    {
        ...
        public static string X2X(string value, int fromBase, int toBase)
        {
            try
            {
                ...
                {
                    throw new ArgumentException(String.Format("The toBase radix \"{0}\" is not in the range 2..36.",
                                                              toBase));
                }
