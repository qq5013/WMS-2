/*----------------------------------------------------------------
// Copyright (C) 2010 有限公司
// 版权所有。 
//
// 文件名：         DepartmentData.cs
// 文件功能描述：   部门数据
//
// 
// 创建标识：
//
// 修改标识：
// 修改描述：
//
// 修改标识：
// 修改描述：
----------------------------------------------------------------*/

using System;
using Frame.Utils.Service;
using MES.Entity;

namespace MES.BllService.Data
{
    /// <summary>
    ///     部门数据
    /// </summary>
    public class DepartmentData : BaseData<Department>
    {
        public void Update(Department entity)
        {
            try
            {
                if (Service.Exists(c => c.Code == entity.Code && c.DepartmentId != entity.DepartmentId))
                    throw CustomError("Code", "代码不能重复");

                Service.Save(entity);
            }
            catch (Exception ex)
            {
                throw CustomError(ex);
            }
        }

        public void Insert(Department entity)
        {
            try
            {
                if (Service.Exists(c => c.Code == entity.Code))
                    throw CustomError("Code", "代码不能重复");

                Service.Save(entity);
            }
            catch (Exception ex)
            {
                throw CustomError(ex);
            }
        }

        public void Delete(Department entity)
        {
            try
            {
                Service.Delete(entity.GetEntityId());
            }
            catch (Exception ex)
            {
                throw CustomError(ex);
            }
        }

        public void UpdateAdvance(Department entity)
        {
            Department department = Service.GetById(entity.GetEntityId());
            department.Code = entity.Code;
            department.CompanyId = entity.CompanyId;
            department.Description = entity.Description;
            department.Forbidden = entity.Forbidden;
            department.Name = entity.Name;
            department.Principal = entity.Principal;
            Update(department);
        }
    }
}