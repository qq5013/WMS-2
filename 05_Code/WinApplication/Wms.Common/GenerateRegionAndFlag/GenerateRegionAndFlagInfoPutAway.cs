...
{
    public class GenerateRegionAndFlagInfoPutAway : AbstractGenerateRegionAndFlagInfo
    {
        ...
        public override int GenerateRegionAndFlag(int warehouseId, string flagCode, string flagName, string memo, int flagId, int userId, bool isInsert)
        {
            ...
            Int32 result = 0;
            string code = string.Format(ConstantString.PutawayFlag, flagCode);
            if (isInsert)
            {
                ...
                regionAndFlag.RegionFlagType = ServiceHelper.FacadeService.GetDictionaryByCode("8104").DictionaryId;
