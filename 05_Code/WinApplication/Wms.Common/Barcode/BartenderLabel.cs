using System;
using System.Collections;
using System.Diagnostics;
using System.IO;
using ecWMS.Core.Exception;

namespace ecWMS.Common.Barcode
{
    public class BartenderLabel
    {
        private static readonly object Locker = new object();

        public static string ExeFileName = "";
        public static string ExeFilePath = "";
        public static string DataFilePath = "";
        public static string FormatFileName = "";
        public static string DataFileName = "";
        public static string HeadData = "";
        public static string DataFormat = "";
        public static string CommandFormat = @"{0}";
        public static string ParmatersFormat = @"/f={0} /d={1} /p /x";
        public static string Command = "";
        public static string Paramters = "";
        public ArrayList DataStrings;

        public BartenderLabel()
        {
            ExeFileName = @"bartend.exe";
            ExeFilePath = AppDomain.CurrentDomain.BaseDirectory;
            DataFilePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"Labels");

            DataStrings = new ArrayList();
        }

        public virtual void Print()
        {
            // init commond
            string p0 = Path.Combine(ExeFilePath, ExeFileName); ;
            Command = string.Format(CommandFormat, p0);

            // init parameters
            string p1 = Path.Combine(DataFilePath, FormatFileName);
            string p2 = Path.Combine(DataFilePath, DataFileName);
            Paramters = string.Format(ParmatersFormat, p1, p2);

            // Init Data
            string dataFile = Path.Combine(DataFilePath, DataFileName);
            WriteDataFile(dataFile, DataStrings);

            try
            {
                Process ps = new Process();
                ps.StartInfo.FileName = Command;
                ps.StartInfo.Arguments = Paramters;
                ps.Start();
            }
            catch (Exception ex)
            {
                BusinessExceptionHelper.HandleException(ex, true, false);
            }
        }

        public virtual void AppendData(string data)
        {
            if (DataStrings != null)
            {
                DataStrings.Add(data);
            }
        }

        public static void WriteDataFile(string fileName, ArrayList list)
        {
            EnsureLogFolder();
            string file = Path.Combine(DataFilePath, fileName);
            lock (Locker)
            {
                using (StreamWriter sw = new StreamWriter(file, false))
                {
                    sw.WriteLine(HeadData);
                    foreach (string data in list)
                    {
                        sw.WriteLine(data);
                    }
                }
            }
        }

        public static void EnsureLogFolder()
        {
            if (Directory.Exists(DataFilePath) != true)
                Directory.CreateDirectory(DataFilePath);
        }
    }
}
