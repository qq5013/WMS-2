...
namespace Modules.LocationModule
{
    ...
    {
        [CommandHandler("LocationModule.ShowForm")]
        public void ShowLocationListFormHandler(object sender, EventArgs e)
        {
            LocationListForm list = WorkItem.Items.Get<LocationListForm>("LocationListForm");
            if (list == null)
            {
                list = WorkItem.Items.AddNew<LocationListForm>("LocationListForm");
